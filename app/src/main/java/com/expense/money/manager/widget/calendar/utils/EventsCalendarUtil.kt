package com.expense.money.manager.widget.calendar.utils
...
object EventsCalendarUtil {
    ...
    fun updateSelectedDates(c: Calendar) {
        if (datesInSelectedRange.contains(getDateString(c, DD_MM_YYYY))) datesInSelectedRange.remove(
            getDateString(c, DD_MM_YYYY)
        )
